...
export default function SearchPage() {
  ...
  const {
    ...
    queryFn: ({ pageParam = 1 }) => searchContent(filters, pageParam),
    initialPageParam: 1,
            ...
            {isLoading ? (
              ...
              <div className="space-y-6">
                <p className="text-muted-foreground">
                  {searchResults.length} {searchResults.length === 1 ? 'result' : 'results'} found {filters.query && ` for "${filters.query}"`}
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {data.pages.map((page, i) => (
                    <Fragment key={i}>
                      {page.items.map((article) => (
                        <ArticleCard key={article.id} article={article} showGroup={true} pastelColor={getColorFromId(article.id)} />
                      ))}
                    </Fragment>
                  ))}
                </div>
