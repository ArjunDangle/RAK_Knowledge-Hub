...
import { SearchFilters, Article, Subsection, UpdateEntry, 
  Group, Tag, GroupInfo, ContentItem, PageTreeNode, PaginatedResponse } from '../types/content';
 ...
 export type ArticleSubmissionStatus = "PENDING_REVIEW" |
  "REJECTED" | "PUBLISHED";
 ...
 // --- AUTH FUNCTIONS ---
 // --- FIX 1: Replaced 'any' with a specific interface ---
 ...
 export async function getPageContents(parentId: string, page: number = 1): Promise<PaginatedResponse<ContentItem>> { 
   return apiFetch<PaginatedResponse<ContentItem>>(`/contents/by-parent/${parentId}?page=${page}&pageSize=10`);
  }
 ...
 export async function getRelatedArticles(tags: Tag[], currentId: string, limit = 4): Promise<Article[]> {
  ...
  const allArticles = await apiFetch<Article[]>(`/search?q=${tagNames.join(' ')}`);
   return allArticles.filter(a => a.id !== currentId).slice(0, limit);
  }
 
 export async function searchContent(filters: SearchFilters, page: number = 1): Promise<PaginatedResponse<Article>> {
 ...
 export async function getWhatsNew(): Promise<UpdateEntry[]> {
   ...
   // --- FIX 2: Removed the unnecessary '(article as any)' cast ---
  return articles.map(article => ({ id: article.id, type: 'update', title: article.title, summary: article.description || article.excerpt, date: article.updatedAt, articleSlug: article.slug, group: article.group as Group }));
  }
 
 // --- CMS & ADMIN FUNCTIONS ---
 export async function getPageDetailsForEdit(pageId: string): Promise<PageDetailResponse> {
     return apiFetch<PageDetailResponse>(`/cms/admin/edit-details/${pageId}`);
 }
 
 export async function getArticleForPreview(pageId: string): Promise<Article |
  null> {
 ...
 export async function getContentIndex(parentId?: string): Promise<ContentNode[]> {
  ...
  `/cms/admin/content-index?parent_id=${parentId}` : '/cms/admin/content-index';
