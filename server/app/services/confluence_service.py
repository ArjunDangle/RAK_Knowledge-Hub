...
class ConfluenceService:
    ...
    def _transform_page_to_article(self, page_data: dict, is_admin_view: bool = False) -> Optional[Article]:
        ...
        read_minutes = max(1, round(word_count / 200)) # Calculating read_minutes here
        ...
        article_data = {
            ...
            "excerpt": excerpt,
            "description": excerpt, # Use excerpt as description for old model
            "html": html_content,
            "tags": tags,
            "group": group_slug,
            ...
            "views": 0,
            "readMinutes": read_minutes, # Use the calculated value
    ...
    async def get_article_by_id_hybrid(self, page_id: str) -> Optional[dict]:
        ...
        # Step 2: Fetch only the heavy 
        # HTML content from Confluence
        try:
            page_content_data = self.confluence.get_page_by_id(page_id, expand="body.view,version")
            html_content = page_content_data.get("body", {}).get("view", {}).get("value", "")
            # --- CALCULATE READMINUTES DYNAMICALLY ---
            plain_text = self._get_plain_text(html_content)
            word_count = len(plain_text.split())
        ...
        merged_data = {
            ...
            "readMinutes": read_minutes, # <-- Use the new dynamically calculated value
    ...
    async def create_page_for_review(self, page_data: PageCreate, author_id: int, author_name: str) -> dict:
        ...
        try:
            ...
            await db.page.create(data={
                ...
                # readMinutes will use its default value
            })
            ...
            time.sleep(1) # Give Confluence a moment to 
